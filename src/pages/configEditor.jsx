...
export default function ConfigEditor() {
  ...
  const [prompts, setPrompts] = useState(null),
    ...
    botPrompts = [
      ...
      { title: 'Amounts to pass deal immidiately:', name: "currencyValues", prompt: prompts?.botPrompts?.currencyValues },
    ],
    ...
    programmingVariables = [
      { title: 'FAQ public link for managers:',  name: "faqGroup", prompt: prompts?.programmingVariables?.faqGroup },
      { title: 'Admin public link for managers:', name: "adminGroup", prompt: prompts?.programmingVariables?.adminGroup },
    ...
    const editPromptValue = (e, key) => {
        console.log(e.target.name)
        setPrompts({...prompts, [key]: {...prompts[key], [e.target.name]: e.target.value}})
    }

    const promptDisplay = (title, name, prompt, key, index) => {
        return(
            <div className='flex w-full flex-col items-start mt-4' key={index}>
    ...
    const addGroup = (e, key, value, newValue) => {
      e.preventDefault();
      if(!newValue) return;
      setPrompts({...prompts, [key]: {...prompts[key], [value]: [...prompts.programmingVariables[value], newValue.current.value] }});
      ...
      {prompts && 
        <form onSubmit={updatePrompts} className='flex flex-col items-center w-full'>
          ...
          {configParams.map((item, index) => (
            promptDisplay(item.title, item.name, item.prompt, "botPrompts", index)
          ))}
          ...
          <button className='ml-2 bg-gray-200 py-2 px-4' onClick={addItemToKeysAndGreetings}>Add Greeting with keywords</button>
          ...
          <div className='w-full flex my-2 justify-between'>
            <input className='w-[calc(100%_-_140px)] border-b-2 border-l-2 px-2 border-gray-400' placeholder='Add group' onChange={e => addGroupToJoinInput.current.value = e.target.value} ref={addGroupToJoinInput} />
            <button className='bg-gray-200 py-2 px-4 self-start ml-2 w-[120px]' onClick={e => addGroup(e, "programmingVariables", "groupsToAddBots", addGroupToJoinInput)}>add group</button>
          </div>
          ...
          <div className='w-full flex my-2 justify-between'>
            <input className='w-[calc(100%_-_140px)] border-b-2 border-l-2 px-2 border-gray-400' placeholder='Add group' onChange={e => addGroupInput.current.value = e.target.value} ref={addGroupInput} />
            <button className='bg-gray-200 py-2 px-4 self-start my-2 ml-2 w-[120px]' onClick={e => addGroup(e, "programmingVariables", "allowedGroups", addGroupInput)}>add group</button>
          </div>
            
          {programmingVariables.map((item, index) => (
            promptDisplay(item.title, item.name, item.prompt, "programmingVariables", index)
          ))}

          <button className='mt-4 bg-green-500 py-2 px-4 my-2 text-white text-lg rounded-lg self-center'
